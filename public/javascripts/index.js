        ...
        success: function (response) {
            if (response.isLogin == true) {
                ...
                if (response.user.headImg == 'NaN') {
                    $('#loginButton').remove();
                    $('#userHead').prepend(`<a userId="${response.user.id}" userName="${response.user.userName}" class="toPerson" href="/person"><svg viewBox="0 0 1024 1024"><path d="M512 538.1c130.9 0 237-106.1 237-237s-106.1-237-237-237-237 106.1-237 237 106.1 237 237 237z m0 110.6c-218.2 0-395.1 69.7-395.1 155.6S293.8 960 512 960s395.1-69.7 395.1-155.6S730.2 648.7 512 648.7z" fill="#707070"></path></svg></a>`);
                } else {
                    $('#loginButton').remove();
                    $('#userHead').prepend(`<a userId="${response.user.id}" userName="${response.user.userName}" class="toPerson" href="/person"><svg viewBox="0 0 1024 1024"><path d="M512 538.1c130.9 0 237-106.1 237-237s-106.1-237-237-237-237 106.1-237 237 106.1 237 237 237z m0 110.6c-218.2 0-395.1 69.7-395.1 155.6S293.8 960 512 960s395.1-69.7 395.1-155.6S730.2 648.7 512 648.7z" fill="#707070"></path></svg></a>`);
                    $('.toPerson>svg').remove();
                    if (response.user.headImg == 'NaN.png') {
                        $('.toPerson').prepend('<img onerror=\'picError(this)\'  src="/head/staticIMG/' + response.user.headImg + '">');
                    } else {
                        $('.toPerson').prepend('<img onerror=\'picError(this)\'  src="/head/' + response.user.headImg + '">');
                    }
                }
